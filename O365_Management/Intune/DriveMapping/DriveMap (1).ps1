$taskName = "MapDrives"
$scriptSavePath = "$env:ProgramData\IntuneInstalls\Drives"
$PS_ScriptPath = Join-Path -Path $scriptSavePath -ChildPath "MapDrives.ps1"
...
# Write the drive mapping script
@"
net use G: "\\wc-files\tdrive$" /persistent:yes
...
Register-ScheduledTask -TaskName $taskName -Trigger $trigger1,$trigger2,$trigger3 -Action $action -Principal $principal -Settings $settings -Description "Map drives on logon and network change" -Force
